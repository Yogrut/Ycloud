...
桌面端右键菜单跟随指针，并区分文件、已选项目和列表空白区域；桌面文件列表支持方向式拖选，向下增加选中行、向上取消已选行，不绘制额外选择框，也不触发文字选择或外部文件拖入上传。移动端不启用拖选，选中项目后显示横向操作面板。批量操作保留后端逐项原子语义，部分成功时展示失败项目明细。管理后台仍由后续阶段迁移。
