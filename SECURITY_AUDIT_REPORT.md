...
容器部署基线新增多阶段 Linux `Dockerfile`、默认发布 `18473:18473` 的单一 `compose.yaml`、按注释启用的 HTTPS 代理变量、非 root UID/GID 10001、Capability 全移除、`no-new-privileges` 和 PID 上限。配置主密钥默认随数据卷持久化，外部密钥保持为可选覆盖项。CI 已加入 Compose 模型解析、镜像构建、用户元数据检查以及容器进程稳定运行验证。
