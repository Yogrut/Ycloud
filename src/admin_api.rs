...
pub async fn create_share(
    ...
    let password_hash = hash_optional_password(&state, body.password).await?;
    let share = Share {
...
pub async fn update_share(
    ...
    let password_hash = hash_changed_password(&state, body.password).await?;
    let result = state
        .update_config(move |config| {
...
pub async fn update_admin_account(
    ...
    let global_web_password_hash = hash_changed_password(&state, body.global_web_password).await?;
...
fn validate_password(password: &str) -> AppResult<()> {
    if password.is_empty() || password.len() > 1_024 {
        Err(AppError::BadRequest(
            "Password must contain 1-1024 bytes".into(),
        ))
    } else {
        Ok(())
    }
